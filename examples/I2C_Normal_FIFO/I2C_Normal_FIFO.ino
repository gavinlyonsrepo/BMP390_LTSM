/*!
	@file    I2C_Normal_FIFO.ino
	@brief   Arduino C++ BMP390 library test file, FIFO + interrupt, I2C hardware.
...
// FIFO_WATERMARK (200 bytes) / 7 bytes per frame = 28 frames
//   Interrupt fires every: 28 frames × 320 ms = ~9.14S
// FIFO capacity check:
...
#define FIFO_WATERMARK  200       // Fire interrupt when FIFO reaches 200 bytes
																  ...
																  // → ~28 frames before interrupt fires every ~9.14 Seconds
								  								// ODR 320 mS (50Hz) (FIFO_WATERMARK / 7) * 0.32f);
...
{
		Serial.println("--- START BMP390 SPI FIFO + Interrupt ---");
...
void FIFOSetup(void)
{
		...
		// ---- ODR and watermark ----
		// Changing FIFO_WATERMARK or ODR changes the interrupt rate:
		bmp390.setODR(BMP390_Sensor::ODR_e::ODR_3_1_Hz); // 320mS
		bmp390.setFifoWatermark(FIFO_WATERMARK);
		snprintf(SerialBuffer, sizeof(SerialBuffer),
			"FIFO watermark : %u bytes (%u frames, fires every ~%.1f s)",
			FIFO_WATERMARK,
			FIFO_WATERMARK / 7,
			(FIFO_WATERMARK / 7) * 0.32f);
		Serial.println(SerialBuffer);
